...
- Clarified daemon peer discovery: addnode= adds peers in addition to the DNS-discovered peers rather than replacing them. For an isolated node, use connect= or set dnsseed=0.
- [W8] Windows: wait for daemon to fully exit before quitting (fixes chain rescan on every restart)
- Security hardening across the application. Updating is recommended.
